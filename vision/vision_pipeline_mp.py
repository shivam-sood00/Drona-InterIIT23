...
class VisionPipeline():
    ...
    def get_components_parallel(self, x,y, component_id, component_size, counter,start_x,start_y,end_x,end_y, comp_points, depth_full_distances, DEPTH_DETECTION_THRESHOLD, DEPTH_SEARCH_REGION):
            ...
            for xx in range(x_t-2, x_t+4,2):
                for yy in range(y_t-2, y_t+4,2):
                    if (x_t==xx and y_t==yy) or yy<start_y or xx<start_x or xx>=end_x or yy>=end_y:
                        continue
                    if component_id[xx-start_x][yy-start_y] == -1 and depth_full_distances[yy][xx]<DEPTH_DETECTION_THRESHOLD:
                        if abs(depth_full_distances[y_t][x_t]-depth_full_distances[yy][xx])< 0.025 and abs(xx-x) < 3*DEPTH_SEARCH_REGION and abs(yy-y) < 3*DEPTH_SEARCH_REGION:
    ...
    @ray.remote
    # estimate_drone_center_parallel.remote(self.depth_full_distances.copy(), start_x, end_x, start_y, end_y, self.DEPTH_DETECTION_THRESHOLD, self.DEPTH_SEARCH_REGION)
    def estimate_drone_center_parallel(depth_full_distances, start_x, end_x, start_y, end_y, DEPTH_DETECTION_THRESHOLD, DEPTH_SEARCH_REGION):
            ...
            # print(self.depth_frame_full.get_distance(x,y))
                if component_id[x-start_x][y-start_y] == -1 and depth_full_distances[y][x]>1.0 and depth_full_distances[y][x]<2.2: # parameters
                    ...
                    # coordinates.append(self.get_components_parallel(self, x,y, component_id, component_size, counter,start_x,start_y,end_x,end_y, comp_points, depth_full_distances, DEPTH_DETECTION_THRESHOLD, DEPTH_SEARCH_REGION))
                    coordinates.append((x_mean//component_size[counter], y_mean//component_size[counter]))
                    counter+=1
    ...
    def cam_process(self):
        ...
        markers_use_depth = [False, False]
        markers_depth_reason = [0, 0]

        for i, (key,marker_corners) in enumerate(marker_corners_all.items()):
            if marker_corners is None:
                markers_use_depth[i] = True
                markers_depth_reason[i] = 1
        ...
        if markers_use_depth[0] and markers_use_depth[1]:

            pose_estimates = [1, 1]
            parallel_centers = []

            self.DEPTH_SEARCH_REGION = 50
            pose_pixel = rs.rs2_project_point_to_pixel(self.depth_intrinsics, self.previous_pose[list(marker_corners_all.keys())[0]])
            if math.isnan(pose_pixel[0]):
                # return None
                pose_estimates[0] = None

            if pose_estimates[0] is not None:          
                start_x = max(0,int(pose_pixel[0])-self.DEPTH_SEARCH_REGION)
                end_x = min(int(pose_pixel[0])+self.DEPTH_SEARCH_REGION, int(self.depth_res[1]))
                start_y = max(0,int(pose_pixel[1])-self.DEPTH_SEARCH_REGION)
                end_y = min(int(pose_pixel[1])+self.DEPTH_SEARCH_REGION, int(self.depth_res[0]))  

                parallel_centers.append(self.estimate_drone_center_parallel.remote(self.depth_full_distances.copy(), start_x, end_x, start_y, end_y, self.DEPTH_DETECTION_THRESHOLD, self.DEPTH_SEARCH_REGION))


            ##########################
            pose_pixel = rs.rs2_project_point_to_pixel(self.depth_intrinsics, self.previous_pose[list(marker_corners_all.keys())[1]])
            if math.isnan(pose_pixel[0]):
                # return None
                pose_estimates[1] = None

            if pose_estimates[1] is not None:
                start_x = max(0,int(pose_pixel[0])-self.DEPTH_SEARCH_REGION)
                end_x = min(int(pose_pixel[0])+self.DEPTH_SEARCH_REGION, int(self.depth_res[1]))
                start_y = max(0,int(pose_pixel[1])-self.DEPTH_SEARCH_REGION)
                end_y = min(int(pose_pixel[1])+self.DEPTH_SEARCH_REGION, int(self.depth_res[0]))  
            ...
            print("PARALLEL: ", parallel_centers)
            center_estimates = ray.get(parallel_centers)

            #########################
            if len(center_estimates) == 0:
                pass
            elif len(center_estimates) == 1:
                print("ONLY 1")
                if pose_estimates[0] is None:
                    # center_estimates = ray.get(parallel_centers)
                    if center_estimates[0] is None:
                        # return None
                        pose_estimates[1] = None

                    if pose_estimates[1] is not None:                
                        pose_estimates[1] = self.position_from_pixel(self.depth_intrinsics, center_estimates[1][0], center_estimates[1][1], self.depth_frame_full.get_distance(center_estimates[1][0], center_estimates[1][1]), key)
                else:
                    # center_estimates = ray.get(parallel_centers)
                    if center_estimates[0] is None:
                        # return None
                    ...
                    if pose_estimates[0] is not None:                
                        pose_estimates[0] = self.position_from_pixel(self.depth_intrinsics, center_estimates[1][0], center_estimates[1][1], self.depth_frame_full.get_distance(center_estimates[1][0], center_estimates[1][1]), key)

            else:
                print("HERE")
                print("BOTH")
                # center_estimates = ray.get(parallel_centers)
                print("PARALLEL PROCESSING RESULT: ", center_estimates)

                if center_estimates[0] is None:
                    pose_estimates[0] = None

                if pose_estimates[0] is not None:               
                    pose_estimates[0] = self.position_from_pixel(self.depth_intrinsics, center_estimates[0][0], center_estimates[0][1], self.depth_frame_full.get_distance(center_estimates[0][0], center_estimates[0][1]), key)

                if center_estimates[1] is None:
                    # return None
                    pose_estimates[1] = None
            ...
            if pose_estimates[0] is None:
                key = list(marker_corners_all.keys())[0]
                if markers_depth_reason[0] == 1:
                    self.counter[key] += 1
                    if self.counter[key] >= 30:
                        flag = 2
                        # cam_queue.append([flag])
                ...
                key = list(marker_corners_all.keys())[0]
                estimated_pose = [pose_estimates[0][0], pose_estimates[0][1], pose_estimates[0][2]]
                self.depth_estimated_pose[key] = [pose_estimates[0][0], pose_estimates[0][1], pose_estimates[0][2]]
                self.estimated_pose_all[key] = estimated_pose

            
            if pose_estimates[1] is None:
                key = list(marker_corners_all.keys())[1]
                if markers_depth_reason[1] == 1:
                    self.counter[key] += 1
                    if self.counter[key] >= 30:
                        flag = 2
                        # cam_queue.append([flag])
                ...
                key = list(marker_corners_all.keys())[1]
                estimated_pose = [pose_estimates[1][0], pose_estimates[1][1], pose_estimates[1][2]]
                self.depth_estimated_pose[key] = [pose_estimates[1][0], pose_estimates[1][1], pose_estimates[1][2]]
                self.estimated_pose_all[key] = estimated_pose


        elif markers_use_depth[0] or markers_use_depth[1]:

            print("IN THIS ELIF")

            key = None
            use_index = -1
            if markers_use_depth[0]:
                use_index = 0
                key = list(marker_corners_all.keys())[0]
            else:
                use_index = 1
            ...
            pose_from_depth = self.pose_estimation(key,use_cam=False, use_depth=True)
            print("IN THIS ELIF: ", pose_from_depth)
            
            if markers_depth_reason[use_index] == 1:
                if len(pose_from_depth):
                    estimated_pose = [pose_from_depth[0], pose_from_depth[1], pose_from_depth[2]]
                    self.depth_estimated_pose[key] = [pose_from_depth[0], pose_from_depth[1], pose_from_depth[2]]
                    self.estimated_pose_all[key] = estimated_pose
            ...
            # self.estimate_drone_center_parallel(depth_full_distances, start_x, end_x, start_y, end_y, DEPTH_DETECTION_THRESHOLD, DEPTH_SEARCH_REGION):
            pass

        print("FULL CAM PROCESS TIME: ", time.time() - start_time)
